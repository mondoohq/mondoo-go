...
type ApplyRemediationInput struct {
	...
	AssetMrn String `json:"assetMrn" tfgen:"required=1"`
	// The finding to remediate (CVE, advisory, or check). (Required.)
	FindingMrn String `json:"findingMrn" tfgen:"required=1"`
	// Every mechanism the fix is expressible in — the same ids a RemediationScript reports. Pass them all; resolution answers with the one to use here, and prefers the durable path where an actuator offers several. (Required.)
	Mechanisms []String `json:"mechanisms" tfgen:"required=1"`
	...
	// Provider-side group to target, when the selected capability is GROUP-granular and the caller wants a specific one. Omitted lets the delivery path choose, which for Intune means device-direct where the tenant allows it and the device's own group otherwise. (Optional.)
...
type FleetScanRunInput struct {
	...
	// When true, the workflow skips its post-poll DeleteProviderArtifact step so an operator can inspect the per-scan provider artifact (e.g. Intune deviceHealthScript) for debugging. The deferred Cleanup activity also honours this flag. Defaults to false. (Optional.)
