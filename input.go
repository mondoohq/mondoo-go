...
type MoonRabbitConfigurationOptionsInput struct {
	// Base URL of the moon-rabbit service's GraphQL endpoint, e.g. https://moonrabbit.example:8099/query. (Required.)
	Endpoint String `json:"endpoint" tfgen:"required=1"`

	// Bearer token the service authenticates every request with. Required on create; on update, send it only when the endpoint changes or the token is rotated. (Optional.)
	Token *String `json:"token,omitempty" tfgen:"required=0"`
...
type MsIntuneConfigurationOptionsInput struct {
	// The MS Intune tenant ID. (Required.)
	TenantId String `json:"tenantId" tfgen:"required=1"`
	// The MS Intune client ID. (Required.)
	ClientId String `json:"clientId" tfgen:"required=1"`

	// The PEM certificate for certificate-based authentication. (Optional.)
	Certificate *String `json:"certificate,omitempty" tfgen:"required=0"`
	// The client secret for password-based authentication. (Optional.)
