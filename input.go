...
type ApplyIntuneRemediationInput struct {
	...
	// The Intune device group ID to target. Optional: when omitted (or empty) and assetMrn is provided, the remediation is triggered on-demand against that specific device (no group assignment), falling back to the device's own group when on-demand is unavailable on the tenant. (Optional.)
