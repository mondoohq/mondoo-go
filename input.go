...
// ActuatorCapabilityInput represents one declared delivery path.
type ActuatorCapabilityInput struct {
	...
	// What a change delivered this way lands on. Omitted means "not stated". (Optional.)
	Granularity *ActuatorGranularity `json:"granularity,omitempty" tfgen:"required=0"`
}
...
type ApplyOpsiRemediationInput struct {
	...
	// Target OS of the client: "linux" or "windows". Selects the remediation script flavour the opsi plugin generates. (Required.)
...
type ApplyRemediationInput struct {
	// The asset the fix is being applied from. (Required.)
	AssetMrn String `json:"assetMrn" tfgen:"required=1"`

	// The finding to remediate (CVE, advisory, or check). Exactly one of `findingMrn` or `governedMrn` must be set. Nullable only because the governed-target form exists; a finding-driven call still requires it. (Optional.)
	FindingMrn *String `json:"findingMrn,omitempty" tfgen:"required=0"`
	// The governed resource to REMOVE from the asset — a software or AI-component MRN that governance has a decision on (ADR-098). Removing a denied component is an ordinary change, so it resolves through the same actuator order, returns the same statuses, and obeys the same group-target rule as a finding fix. It is a sibling of `findingMrn` rather than a reuse of it because a removal is addressed by WHAT IS DENIED, not by the row that reports it — a governed component has no finding today, and will still be addressed this way when it has one. Exactly one of `findingMrn` or `governedMrn` must be set. (Optional.)
	GovernedMrn *String `json:"governedMrn,omitempty" tfgen:"required=0"`
	// Every mechanism the fix is expressible in — the same ids a RemediationScript reports. Pass them all; resolution answers with the one to use here, and prefers the durable path where an actuator offers several. Required with `findingMrn`, whose caller holds the fix and therefore knows them. OPTIONAL with `governedMrn`, where the server derives them from what the asset actually has — a caller cannot know whether a denied product is an apt package here and a winget id there, and guessing would route the removal to an actuator that cannot deliver it. Supplying them alongside `governedMrn` narrows that derivation ("only what Ansible could deliver"); it never overrides what the component is. (Optional.)
...
type SetDeclaredActuatorInput struct {
	...
	Name String `json:"name" tfgen:"required=1"`
	// What this system can deliver, each path with its own durability and blast radius. Asserted by the operator, not probed — so leaving a path's durability or granularity out means "not stated", which readers treat as unknown rather than as ONE_SHOT or ASSET. (Required.)
