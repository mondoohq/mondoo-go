...
const (
	GovernanceStateDiscovered GovernanceState = "DISCOVERED"   // No decision applies. The default, and never stored — an entity is discovered precisely when no decision row resolves for it.
	GovernanceStateAllowed    GovernanceState = "ALLOWED"      // Sanctioned. Its presence is expected.
	GovernanceStateOutOfScope GovernanceState = "OUT_OF_SCOPE" // Deliberately not governed: someone looked and chose not to deal with it. A decision, unlike DISCOVERED, which is the absence of one — this is what stops an entity from coming back for triage.
	GovernanceStateDenied     GovernanceState = "DENIED"       // Not sanctioned; its presence is a violation. The only state that can additionally be flagged for removal.
)
